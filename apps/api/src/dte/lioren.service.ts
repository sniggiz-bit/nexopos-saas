...
export class LiorenService {
  ...
  async emitirBoleta(saleId: string) {
    try {
      this.logger.log(
        `[Lioren] Iniciando emisión de DTE para venta ${saleId}...`,
      );
      ...
      if (!token) {
        this.logger.warn(
          `[Lioren] Tenant ${sale.tenantId} no tiene token configurado y no hay token por defecto.`,
        );
      ...
      const liorenPayment = LiorenHelper.mapPaymentMethod(
        mainPayment?.paymentMethod as any,
      );
      ...
      if (responseData && responseData.folio && responseData.url_pdf) {
        ...
        const errorMsg =
          responseData?.message || 'Respuesta inválida de Lioren';
        throw new Error(errorMsg);
      }
    } catch (error) {
      this.logger.error(
        `[Lioren] ❌ Error emitiendo DTE para venta ${saleId}: ${error.message}`,
      );
      await this.prisma.sale
        ...
        .catch((e) =>
          this.logger.error(`Error actualizando dteStatus: ${e.message}`),
        );
